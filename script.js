...
/* ══ ROAD PERKS ══════════════════════════════════════════ */
// Only shield and doubler remain; doubler now activates on 2nd consecutive near-miss via streak
const PERK_DEFS = {
  shield:  { icon:'🛡️', color:'#39ff8a', label:'SHIELD',   dur:5000 },
  doubler: { icon:'×2',  color:'#ffd700', label:'×2 SCORE', dur:8000 },
};
let roadPerks = [];
let activePerk = null;
...
function drawSpeedometer(speedKmh){
  ...
  for(let i=0;i<=8;i++){
    ...
    speedoCtx.lineWidth=1; speedoCtx.strokeStyle='rgba(255,255,255,0.25)'; speedoCtx.stroke();
  }
  ...
  try {
    ...
    if(!localStorage.getItem(MIGRATION_KEY)){
      try {
        ...
        const oldText=await fetch(`${STOREGIT_BASE}/api/download?name=${encodeURIComponent(OLD_FILE)}`,{headers:{'X-API-Key':k}}).then(r=>{if(!r.ok)throw new Error(r.status);return r.text();});
        ...
        if(Array.isArray(oldEntries)&&oldEntries.length){
          ...
          try { const files=await lbRequest('GET','files').catch(()=>[]); const old=Array.isArray(files)?files.find(f=>f.name===OLD_FILE||f.originalName===OLD_FILE):null; if(old) await lbRequest('DELETE',`files/${old.sha||old.id||old.name}`).catch(()=>{}); } catch {}
        }
      } catch {}
...
function collectPerk(type){
  ...
  const ph=$('perk-hud'); if(ph){ ph.textContent=def.icon; ph.classList.add('active'); }
  pops.push({x:GW/2,y:GH_BASE*0.38,a:1.5,t:def.label+'!',c:def.color});
  if(S.vibrateOn&&navigator.vibrate) navigator.vibrate([20,10,30]);
}
function clearPerk(){
  activePerk=null;
  const ph=$('perk-hud'); if(ph) ph.classList.remove('active');
}
...
function drawPerks(){
  roadPerks.forEach(p=>{
    ...
    grad.addColorStop(0,def.color+'44'); grad.addColorStop(1,def.color+'08');
    ctx.globalAlpha=1;
    ctx.fillStyle=grad;
    ...
    if(p.type==='doubler'){
      ctx.font="bold 12px 'Orbitron',monospace"; ctx.fillStyle=def.color;
...
function renderAchievementsTab(){
  ...
  el.innerHTML=ACHIEVEMENT_DEFS.map(def=>{
    ...
    return `<div class="ach-row${done?' ach-done':''}">
      <span class="ach-icon">${def.label}</span>
      <span class="ach-text"><span class="ach-label">${def.name}</span><span class="ach-desc">${def.desc}</span></span>
      ${done?'<span class="ach-check">✓</span>':'<span class="ach-check" style="opacity:0.2">🔒</span>'}
    </div>`;
  }).join('');
}
...
function drawDailyHUD(){
  ...
  const bx=8, by=12, bw=GW-16, bh=7;
  ctx.save();
  // bg track
  ...
  if(frac>0){
    ...
    grad.addColorStop(0,'#2255cc'); grad.addColorStop(1,'#66aaff');
    ctx.fillStyle=grad;
...
function loop(ts){
  ...
  if(STATE==='playing'){
    ...
    for(let i=traffic.length-1;i>=0;i--){
      ...
      // Near miss — score multiplier starts at 2nd consecutive miss
...
async function loadEngineBuffer(){
  ...
  _engineLoading=true;
  try { const ac=getAudioCtx(); const resp=await fetch('sounds/engine.mp3'); const ab=await resp.arrayBuffer(); engineBuffer=await ac.decodeAudioData(ab); } catch(e){ console.warn('engine.mp3 failed',e); }
...
$('lb-submit-btn').addEventListener('click', async ()=>{
  ...
  $('lb-submit-btn').disabled=true; $('lb-submit-status').textContent='Looking up\u2026';
